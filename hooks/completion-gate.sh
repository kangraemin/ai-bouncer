  ...
  if [ "$PHASE_COUNT" -gt 0 ]; then
    ...
    if [ "$ALL_DONE" = "false" ]; then
      ...
      case "$CG_COUNT" in
        ...
        3)  ESC="[3회 차단] 진행 여부를 묻는 것 자체가 dev-bounce 규칙(SKILL 3-1) 위반이다. 다음 step 코드를 작성하라." ;;
        ...
        6)  ESC="[6회 차단] 멈춤 시도가 반복되고 있다. 남은 미완료 step을 지금 구현하라." ;;
        7)  ESC="[7회 차단] 멈춤 시도가 계속되고 있다. 다음 미완료 step을 지금 바로 구현하라." ;;
