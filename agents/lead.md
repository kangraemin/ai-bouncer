...
`{TASK_DIR}/plan.md` 읽어 아래 기준으로 종합 평가:
...
판단 후 출력:
```
[TEAM:solo]   — 간단한 수정, 1-3개 파일
[TEAM:duo]    — 중간 복잡도, Dev 1명 스폰
[TEAM:team]   — 고복잡도, Dev + QA 스폰
```

**파일 수만으로 판단하지 않는다.** 복잡한 로직, 새 모듈 도입, 크로스 컴포넌트 변경이면 solo여도 duo/team 선택 가능.
...
s['dev_phases'] = {
    "1": {
        ...
        "steps": {
            "1": {"title": "...", "test_defined": False, "passed": False,
...
`[ALL_STEPS:완료]` 출력 → commands/dev.md가 Phase 4(verifier) 진행
